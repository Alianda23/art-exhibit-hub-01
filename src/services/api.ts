...
export async function createCancellationRequest(bookingId: string, reason: string) {
  try {
    ...
    return { error: 'Failed to create cancellation request' };
  }
}

export async function getAllCancellationRequests() {
  try {
    console.log('=== Starting cancellation requests fetch ===');
    console.log('Token exists:', !!getToken());
    console.log('Is admin:', isAdmin());
    console.log('Token value:', getToken()?.substring(0, 20) + '...');
    console.log('Admin flag from localStorage:', localStorage.getItem('isAdmin'));
...
export async function updateCancellationStatus(cancellationId: string, status: string, adminNotes?: string) {
  try {
    console.log(`Updating cancellation ${cancellationId} to status ${status}`);  // Debug log
    const response = await authFetch(`/cancellations/${cancellationId}/status`, {
      method: 'POST',
      body: JSON.stringify({
        status: status,
    ...
    console.log('Update cancellation response:', response);  // Debug log
    return response;
  } catch (error) {
    console.error('Error updating cancellation status:', error);
    return { error: 'Failed to update cancellation status' };
  }
}

export async function getUserCancellationRequests(userId: string) {
  try {
    const response = await authFetch(`/user/${userId}/cancellations`);
    return response;
  } catch (error) {
    console.error('Error fetching user cancellation requests:', error);
    return { error: 'Failed to fetch user cancellation requests' };
  }
}
