...
from cancellation import create_cancellation_request, get_all_cancellation_requests, update_cancellation_status, get_user_cancellation_requests
...
class RequestHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # Handle GET /user/{user_id}/cancellations - user's cancellation requests
        elif path.startswith('/user/') and path.endswith('/cancellations') and len(path.split('/')) == 4:
            user_id = path.split('/')[2]
            print(f"Processing GET /user/{user_id}/cancellations request")
            ...
            if not is_admin and requesting_user_id != user_id:
                self._set_response(403)
                self.wfile.write(json_dumps({"error": "Access denied - you can only view your own cancellation requests"}).encode())
                return
            
            # Get user's cancellation requests
            response = get_user_cancellation_requests(user_id)
            self._set_response()
            self.wfile.write(json_dumps(response).encode())
            return
    ...
    def do_POST(self):
        ...
        # Create cancellation request
            ...
            print("Processing POST /cancellations request")
            ...
            # Create cancellation request
            response = create_cancellation_request(
                user_id, 
        ...
        # Update cancellation status (admin only)
        elif path.startswith('/cancellations/') and path.endswith('/status'):
            cancellation_id = path.split('/')[2]
            print(f"Processing POST /cancellations/{cancellation_id}/status request")
            ...
            if 'status' not in post_data:
                self._set_response(400)
                self.wfile.write(json_dumps({"error": "Status is required"}).encode())
                return
            
            # Update cancellation status
            response = update_cancellation_status(
                cancellation_id,
                post_data['status'],
                admin_id,
                post_data.get('admin_notes')
            )
